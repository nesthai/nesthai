...
function formToProperty(form) {
  ...
  return {
    ...
    seo: {
      title: form.seoTitle.value.trim() || (title + ' | Nes Thai Business'),
      description: form.seoDescription.value.trim(),
      canonicalUrl: 'https://www.nesthai.com/' + slug + '.html'
    }
  };
}
...
window.duplicateProperty = async function(id) {
  ...
  if (dup.seo) dup.seo.canonicalUrl = 'https://www.nesthai.com/' + dup.slug + '.html';
  props.push(dup);
  saveProperties(props);
  renderPropertiesList();
};
